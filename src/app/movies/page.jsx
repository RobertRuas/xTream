...
export default function MoviesPage() {
    ...
    useEffect(() => {
        if (!userData) return;
        fetchXtreamData(userData, 'get_vod_categories').then(setCategories).catch(console.error);
    }, [userData]);

    useEffect(() => {
        if (!userData) return;
        setLoading(true);
        fetchXtreamData(userData, 'get_vod_streams', activeCategory ? `&category_id=${activeCategory}` : '')
        ...
        <div className="content-area">
            ...
            {loading ? (
                ...
                <div className="content-grid">
                    {items.map(item => <MediaCard key={item.stream_id} item={item} type="vod" onPlay={handlePlay} />)}
                </div>
            )}
        </div>
    );
